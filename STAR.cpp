...
/////////////////////////////////////////////////////////////////////////////////////////////////START
    ...
    if (P->outSAMmode != "None") {//open SAM file and write header
        ...
        if (P->outSAMattrRG!="") {//optional read group header
            samHeaderStream << "@RG";
            for (uint ii=0;ii<P->outSAMattrRGline.size(); ii++) {
                samHeaderStream << "\t" << P->outSAMattrRGline.at(ii);
            };
            samHeaderStream << "\n";
        };
        ...
        if (!P->outBAMbool) {//
            *P->inOut->outSAM << samHeaderStream.str();
        } else if (P->outBAMunsorted){
            outBAMwriteHeader(P->inOut->outBAMfileUnsorted,samHeaderStream.str(),P->chrName,P->chrLength);
        };
    };
