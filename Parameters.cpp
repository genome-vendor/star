...
////////////////////////////////////////////////////// Calculate and check parameters
    ...
    if (outStd=="Log") {
        if (runMode=="alignReads" && outSAMmode != "None") {//open SAM file and write header
            if (outSAMtype.at(0)=="BAM") {
                outBAMcompression=0;//TODO make this input parameter
                outBAMbool=true;
                ...
                for (uint32 ii=1; ii<outSAMtype.size(); ii++) {
                    if (outSAMtype.at(ii)=="Unsorted") {
                        outBAMunsorted=true;
                    } else if (outSAMtype.at(ii)=="SortCoordinate") {
                        outBAMcoord=true;
                    } else {
                        ostringstream errOut;
                        errOut <<"EXITING because of fatal input ERROR: unknown value for the word " <<ii+1<<" of outSAMtype: "<< outSAMtype.at(ii) <<"\n";
                        errOut <<"SOLUTION: re-run STAR with one of the allowed values of outSAMtype: Unsorted or SortCoordinate\n"<<flush;
                        exitWithError(errOut.str(), std::cerr, inOut->logMain, EXIT_CODE_PARAMETER, *this);                                    
                    };
                };
                ...
                if (outBAMcoord) {
                    inOut->outBAMfileCoord = bgzf_open((outFileNamePrefix + "Aligned.out.SortCoordinate.bam").c_str(),("w"+to_string((long long) outBAMcompression)).c_str());
                };                
    ...
    if (outSAMattrRGline.at(0)!="-") {
        outSAMattrRG=outSAMattrRGline.at(0);
        if (outSAMattrRG.substr(0,3)!="ID:") {
            ostringstream errOut;
            errOut <<"EXITING because of FATAL INPUT ERROR: the first word of --outSAMattrRGline="<<outSAMattrRGline.at(0)<<" does not start with ID:xxx read group identifier\n";
            errOut <<"SOLUTION: re-run STAR with --outSAMattrRGline that starts with ID:xxx\n";
            exitWithError(errOut.str(), std::cerr, inOut->logMain, EXIT_CODE_PARAMETER, *this);
        };
        outSAMattrRG=outSAMattrRG.substr(3);
    } else {
        outSAMattrRG="";
    };
    ...
    if (outSAMattrRG!="" && !outSAMattrPresent.RG) {
        outSAMattrOrder[outSAMattrN++]=ATTR_RG;
        inOut->logMain << "WARNING --outSAMattrRG defines a read group, therefore STAR will output RG attribute" <<endl;
    } else if (outSAMattrRG=="" && outSAMattrPresent.RG) {
            ostringstream errOut;
    ...
    if (outSAMstrandField=="intronMotif" && !outSAMattrPresent.XS) {
        outSAMattrOrder[outSAMattrN++]=ATTR_XS;
...
//     for (int ii=0;ii<outSAMattrN;ii++) {
//         if        (vAttr1.at(ii)== "NH") {
//             outSAMattrOrder.NH=ii;
//         } else if (vAttr1.at(ii)== "HI") {
//             outSAMattrOrder.HI=ii; 
//         } else if (vAttr1.at(ii)== "AS") {
//             outSAMattrOrder.AS=ii;
//         } else if (vAttr1.at(ii)== "NM") {
//             outSAMattrOrder.NM=ii; 
//         } else if (vAttr1.at(ii)== "MD") {
//             outSAMattrOrder.MD=ii; 
//         } else if (vAttr1.at(ii)== "nM") {
//             outSAMattrOrder.nM=ii; 
//         } else if (vAttr1.at(ii)== "jM") {
//             outSAMattrOrder.jM=ii; 
//         } else if (vAttr1.at(ii)== "jI") {
//             outSAMattrOrder.jI=ii; 
//         } else if (vAttr1.at(ii)== "XS") {
//             outSAMattrOrder.XS=ii;                 
//         } else {
//             ostringstream errOut;
//             errOut <<"EXITING because of FATAL INPUT ERROR: unknown/unimplemented SAM atrribute (tag): "<<vAttr1.at(ii) <<"\n";
//             errOut <<"SOLUTION: re-run STAR with --outSAMattributes that contains only implemented attributes\n";
...
//     };
//     
//     if (outSAMstrandField!="intronMotif" && outSAMattrOrder.XS>0) {
//         inOut->logMain << "WARNING --outSAMattributes contains XS, therefore STAR will use --outSAMstrandField intronMotif" <<endl;
//         outSAMstrandField="intronMotif";
//     } else if (outSAMstrandField=="intronMotif" && outSAMattrOrder.XS==-1) {
//         outSAMattrOrder.XS=outSAMattrN;
//         ++outSAMattrN;
//     };
